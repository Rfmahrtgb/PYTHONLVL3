...
....
/info - изпользуй для просмотра команд бота
....
/start - используй для начало работы бота
....
/skills - используй для добавления skilla
....
/delete - изпользуй для удаления проекта
....
/projects - изпользуй для просмотра всех сохраненых проектов
....
/update_projects - изпользуй для обновления проекта
....
Также ты можешь ввести имя проекта и узнать информацию о нем!""")
    
# Создания и сохранения нового проекта
...
def callback_project(message, data, statuses):
    ...
    bot.register_next_step_handler(message, descreption_name, data=data)
    manager.insert_project([tuple(data)])
    bot.send_message(message.chat.id, "Проект сохранен")

# Описание проекта
def descreption_name(message, data):
    bot.send_message(message.chat.id, "Введит описание проекта:")
    descreption = message.text
    data.append(descreption)
    message.insert_project(tuple(data))
    statues = [x[0] for x in manager.get.statues()]
    bot.send_message(message.chat.id, "Проект сохранен!")


# skills пиряневшиеся в проекте
